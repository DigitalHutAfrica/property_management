...
class RentInvoice(models.Model):
    ...
    def action_prior_payment_alert(self):
        ...
        mail_template = self.env.ref(
            "property_management.payment_prior_alert_mail_template").sudo()
    ...
    def action_payment_reminder_alert(self):
        ...
        records = self.env["rent.invoice"].sudo().search(
            [('payment_state', '=', 'not_paid'),
             ('contract_type_name', '=', 'lease')])
        mail_template = self.env.ref(
            "property_management.payment_followup_alert_mail_template").sudo()
        sender_email = self.env['ir.config_parameter'].sudo().get_param(
            'property_management.alert_mail_sender_email')
        for record in records:
    ...
    def action_invoice_raise_alert_before(self):
        ...
        records = self.env["rent.invoice"].sudo().search(
            [('rent_invoice_id', '=', False),
             ('contract_type_name', '=', 'lease')])
        mail_template = self.env.ref(
            "property_management.invoice_raise_alert_mail_template").sudo()
        internal_user_id = self.env["ir.config_parameter"].sudo().get_param(
            "property_management.internal_user_id")
        ...
        mail_values = {
            ...
            # 'email_to': mails_str,
            'email_from': self.env['ir.config_parameter'].sudo().get_param('property_management.alert_mail_sender_email') or self.env.company.email or ''
        }
        for record in records:
