...
class RentalConfig(models.TransientModel):
    ...
    reminder_days = fields.Integer(string='Days', default=5,
                                   config_parameter='property_management.reminder_days')
    sale_reminder_days = fields.Integer(string="Days ", default=3,
                                        config_parameter='property_management.sale_reminder_days')
    internal_user_id = fields.Many2one("res.users", string="Internal User",
                                       config_parameter='property_management.internal_user_id')
    team_member_ids = fields.Many2many(
        related="internal_user_id.team_member_ids",
        string='Team Members',
        readonly=False)
    property_report_mails = fields.Char(string='Emails',
                                        config_parameter="property_management.property_report_mails")
    ...
    alert_mail_sender_email = fields.Char('Sender Email',
                                          config_parameter="property_management.alert_mail_sender_email")
    ...
    utility_bill_wh_template_id = fields.Many2one('whatsapp.template',
                                                  ...
                                                  config_parameter='property_management.utility_bill_whatsapp_template_id')
    ...
    payment_receipt_wh_template_id = fields.Many2one('whatsapp.template',
                                                     ...
                                                     config_parameter='property_management.payment_receipt_whatsapp_template_id')

    # Bank Details (used in lease agreement reports)
    maintenance_approval_threshold = fields.Monetary(
        string='Maintenance Approval Threshold',
        config_parameter='property_management.maintenance_approval_threshold',
        default=500000,
    )
    bank_name = fields.Char(
        string='Bank Name',
        default='CRDB BANK (T) LTD',
        config_parameter='property_management.bank_name')

    bank_account_usd = fields.Char(
        string='USD Account No.',
        config_parameter='property_management.bank_account_usd')

    bank_account_tzs = fields.Char(
        string='TZS Account No.',
        config_parameter='property_management.bank_account_tzs')
