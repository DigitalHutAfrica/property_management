...
class TenancyDetails(models.Model):
    ...
    def handover_property(self):
        for rec in self:
            view = self.env.ref(
                'property_management.handover_property_details_form_view')
    ...
    def create_utility_bill(self):
        for rec in self:
            view = self.env.ref('property_management.utility_bill_form_view')
            ctx = {
                'default_contract_id': rec.id,
                'default_property_id': rec.property_id.id,
        ...
        if self.payment_term_id.rent_unit == 'Month':
            record = {
                'product_id': self.env.ref(
                    'property_management.property_product_1').id,
    ...
    def action_send_expiry_alert(self):
        ...
        mail_template = self.env.ref(
            "property_management.agreement_expiry_alert_mail_template").sudo()
        sender_email = self.env['ir.config_parameter'].sudo().get_param(
            'property_management.alert_mail_sender_email')
        for rec in records:
            if rec.end_date:
    ...
    def action_send_active_contract(self):
        mail_template = self.env.ref(
            'property_management.active_contract_mail_template').sudo()
        if mail_template:
            mail_template.send_mail(self.id, force_send=True)

    def action_send_tenancy_reminder(self):
        mail_template = self.env.ref(
            'property_management.tenancy_reminder_mail_template').sudo()
        if mail_template:
            mail_template.send_mail(self.id, force_send=True)
    ...
    def send_expiring_contract_list(self):
        ...
        mail_template = self.env.ref(
            "property_management.contract_expiry_list_alert_mail_template").sudo()
        sender_email = self.env['ir.config_parameter'].sudo().get_param(
            'property_management.alert_mail_sender_email')
    ...
    def tenancy_recurring_invoice(self):
        ...
        reminder_days = self.env['ir.config_parameter'].sudo().get_param(
            'property_management.reminder_days')
