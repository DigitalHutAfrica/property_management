...
class UtilityBill(models.Model):
    ...
    def print_utility_bill_report(self):
        return self.env.ref(
            'property_management.action_utility_bill_report').report_action(self)
    ...
    def confirm_uility(self):
        for rec in self:
            bill_mail_template = self.env.ref(
                "property_management.utility_bill_mail_template", raise_if_not_found=False)
            bill_wh_template = self.get_whatsapp_template(
                'property_management.utility_bill_whatsapp_template_id')
            if rec.state == 'draft':
                rec.state = 'posted'
                rec.r_payment_state = 'Unpaid'
    ...
    def action_send_and_print(self):
        template = self.env.ref("property_management.utility_bill_mail_template",
