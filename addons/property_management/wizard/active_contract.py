...
class ActiveContract(models.Model):
    ...
    def action_monthly_month_active(self):
        ...
        if tenancy_id.payment_term_id.rent_unit == "Month":
            term_months = tenancy_id.payment_term_id.month or 0
            if not term_months:
                raise UserError(_(
                    "The payment term \'%s\' has no month interval set. "
                    "Please set the month value on the payment term before creating invoices."
                ) % (tenancy_id.payment_term_id.name or 'Unknown'))
            vals = total_months / term_months
        ...
        unit = tenancy_id.payment_term_id.month or 1
    ...
    def action_quarterly_month_active(self):
        ...
        if tenancy_id.payment_term_id.rent_unit == "Quarter":
            _qmonths = (tenancy_id.payment_term_id.month or 0) * 3
            if not _qmonths:
                raise UserError(_(
                    "The payment term '%s' has no month interval set. "
                    "Please set the month value on the payment term before creating invoices."
                ) % (tenancy_id.payment_term_id.name or 'Unknown'))
            vals = total_months / _qmonths
